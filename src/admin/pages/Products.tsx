...
import { zProduct, type Product as SharedProduct } from '@shared/models/product'
...
const initialProducts: SharedProduct[] = [
  {
    title: 'Toy Car',
    subtitle: 'Mini racer',
    description: 'A small toy car for kids',
    richDescription: 'Durable plastic, bright colors.',
    images: ['https://example.com/image1.jpg'],
    categoryId: 'toys',
    spuId: 'SPU-1001',
    price: { currency: 'CNY', priceYuan: 39.9 },
    stock: 120,
    isActive: true,
    attributes: { material: 'plastic', age: 3 },
    skus: [
      { skuId: 'SKU-1001-red', priceYuan: 39.9, stock: 60, isActive: true },
      { skuId: 'SKU-1001-blue', priceYuan: 39.9, stock: 60, isActive: true },
    ],
    createdAt: nowMs(),
    updatedAt: nowMs(),
  },
  {
    ...
    description: 'Short stories for children',
    richDescription: 'Illustrated pages, large font.',
    images: [],
    categoryId: 'books',
    spuId: 'SPU-1002',
    price: { currency: 'CNY', priceYuan: 59 },
    stock: 80,
    isActive: true,
...
export function Products() {
  const [items, setItems] = useState<SharedProduct[]>(initialProducts)
  ...
  function toAttrRecord(rows: AttrRow): never
  function toAttrRecord(rows: AttrRow[]): Record<string, string | number | boolean> | undefined
  function toAttrRecord(rows: AttrRow[] | AttrRow): any {
  ...
  function onSave() {
    ...
    const p = parseFloat(priceYuan)
    const s = parseInt(stock, 10)
    if (!title.trim()) return setError('Title is required')
    if (!Number.isFinite(p) || p < 0) return setError('Price must be a non-negative number')
    if (!Number.isFinite(s) || s < 0) return setError('Stock must be a non-negative integer')
    ...
    const product: SharedProduct = {
      ...
      price: { currency: 'CNY', priceYuan: Math.round(p * 100) / 100 },
      stock: s,
      isActive,
      ...
      skus: skus.length
        ? skus.map((r) => ({
            skuId: r.skuId,
            priceYuan: Math.round(parseFloat(r.priceYuan) * 100) / 100 || 0,
    ...
    const check = zProduct.safeParse(product)
    if (!check.success) {
      setError(check.error.issues[0]?.message || 'Invalid product')
      return
    }
            ...
            <div className="mt-4 grid grid-cols-1 gap-3">
              ...
              <div className="mt-2">
                ...
                <div className="space-y-2">
                  ...
                  <Button variant="secondary" onClick={() => setSkus([...skus, { skuId: '', priceYuan: '', stock: '', isActive: true }])}>Add SKU</Button>
                </div>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {items.map((p, idx) => (
                <TableRow key={p.spuId || p.title + p.createdAt + idx}>
